...
function findWDAPort() {
  for (const port of WDA_PORTS) {
    try {
      execSync(`curl -sf --max-time 1 http://localhost:${port}/status`, {
        stdio: ["pipe", "pipe", "pipe"],
      });
      return port;
    } catch {
  ...
  throw new Error(
    "Could not find a running WDA instance on any known port (" +
    WDA_PORTS.join(", ") + "). " +
    "Ensure mobile-mcp is running and connected to a device/simulator."
  );
}

function typeViaWDA(value) {
  const port = findWDAPort();
  const wdaHost = `http://localhost:${port}`;

  // Get the active WDA session
  let sessionId;
  try {
    const statusResp = execSync(`curl -sf ${wdaHost}/status`, {
      encoding: "utf8",
      stdio: ["pipe", "pipe", "pipe"],
    });
    ...
    throw new Error(
      "WDA on port " + port + " responded but has no active session."
    );
  }
  ...
  try {
    ...
    throw new Error("WDA typing failed on port " + port + ". Is a text field focused on the device?");
  }
}
