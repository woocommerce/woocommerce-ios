...
extension StripeCardReaderService: CardReaderService {
    ...
    private func getLocalMobileConfiguration(_ reader: StripeTerminal.Reader, options: CardReaderConnectionOptions?) -> Future<LocalMobileConnectionConfiguration, Error> {
        return Future() { [weak self] promise in
            guard let self = self else {
                promise(.failure(CardReaderServiceError.connection()))
                return
