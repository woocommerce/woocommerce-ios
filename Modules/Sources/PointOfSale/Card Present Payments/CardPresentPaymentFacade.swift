...
public protocol CardPresentPaymentFacade: Sendable {
    ...
    @MainActor var paymentEventPublisher: AnyPublisher<CardPresentPaymentEvent, Never> { get }
    ...
    @MainActor var readerConnectionStatusPublisher: AnyPublisher<CardPresentPaymentReaderConnectionStatus, Never> { get }

    /// `cardReaderUpdateStatePublisher` provides the latest software update state for the connected card reader.
    /// This is a long lasting stream, and will not finish during the life of the façade.
    @MainActor var cardReaderUpdateStatePublisher: AnyPublisher<CardReaderSoftwareUpdateState, Never> { get }
    ...
    @MainActor func connectReader(using connectionMethod: CardReaderConnectionMethod) async throws -> CardPresentPaymentReaderConnectionResult

    /// Disconnects the currently connected card reader, if present.
    /// Also cancels any in-progress payment, if possible.
    @MainActor func disconnectReader() async

    /// Starts a software update for the currently connected card reader, if an update is available.
    /// - Throws: `CardPresentPaymentError` for any failures.
    /// - Output: publishes intermediate events on the `paymentEventPublisher` as required.
    @MainActor func updateCardReaderSoftware() async throws
    ...
    /// - Throws: `CardPresentPaymentError` for any failures.
    /// - Output: publishes intermediate events on the `paymentEventPublisher` as required.
    @MainActor func collectPayment(for order: Order,
    ...
    @MainActor func cancelPayment()

    /// Cancels any in-progress payment, returning when complete
    @MainActor func cancelPayment() async throws
    ...
    @MainActor func cancelReconnection() async
}
