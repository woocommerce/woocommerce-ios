...
extension PointOfSaleAggregateModel {
    func startPrinterDiscovery() {
        guard let receiptPrinterService else { return }
        ...
        printerDiscoveryTask = Task {
            do {
                for try await printer in receiptPrinterService.discover() {
                    ...
                    DDLogInfo("Discovered printer: \(printer.id), all printers: \(discoveredPrinters.map(\.id))")
                }
            } catch {
                DDLogError("Discovery failed: \(error)")
    ...
    func connect(printer: PrinterDevice) {
        ...
        receiptPrinterService?.connect(to: printer)
        printerDiscoveryState = nil
    }
