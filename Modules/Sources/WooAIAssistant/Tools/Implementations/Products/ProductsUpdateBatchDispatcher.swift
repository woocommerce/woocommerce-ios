...
struct ProductsUpdateBatchDispatcher {
    ...
    func dispatch(plans: [EntryPlan]) async -> [BatchOutcome] {
        let batches = groupIntoBatches(plans: plans)
    ...
    private func groupIntoBatches(plans: [EntryPlan]) -> [BatchCall] {
    ...
    private func buildBatchBody(_ writes: [PlannedWrite]) -> AnyCodableJSON {
    ...
    private func dispatchBatch(_ batch: BatchCall) async -> BatchOutcome {
    ...
    private func parseBatchResponse(batch: BatchCall,
