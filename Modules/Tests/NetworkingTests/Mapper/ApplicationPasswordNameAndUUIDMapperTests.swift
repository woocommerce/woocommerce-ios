...
final class ApplicationPasswordNameAndUUIDMapperTests: XCTestCase {
    ...
    /// Verifies that GET application password response in data envelope is parsed properly
    ///
    func test_response_with_data_envelope_is_properly_parsed_when_loading_all_application_passwords() throws {
        guard let passwords = mapGetApplicationPasswordsResponseWithDataEnvelope() else {
            XCTFail()
            return
        }
        ...
        XCTAssertEqual(password.name, "test-name")
    }
}
...
private extension ApplicationPasswordNameAndUUIDMapperTests {
    ...
    ///
    func mapGetApplicationPasswordsResponseWithDataEnvelope() -> [ApplicationPasswordNameAndUUID]? {
        guard let response = Loader.contentsOf("get-application-passwords-success-with-data") else {
            return nil
        }
