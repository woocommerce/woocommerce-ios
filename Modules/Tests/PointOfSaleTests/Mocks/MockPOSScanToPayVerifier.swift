...
final class MockPOSScanToPayVerifier: POSScanToPayVerifying {
    ...
    /// Optional hook fired inside the call so tests can observe in-flight state.
    var onCheckPaymentStatusCalled: (@MainActor () -> Void)?

    func checkPaymentStatus() async throws -> POSScanToPayVerificationResult {
        checkPaymentStatusCallCount += 1
        await MainActor.run { onCheckPaymentStatusCalled?() }
