...
final class PointOfSaleDefaultPurchasableItemFetchStrategyTests: XCTestCase {
    ...
    func test_fetchItems_tracks_analytics_for_first_page() async throws {
        // Given
        let totalItems = 42
    ...
    func test_fetchItems_does_not_track_analytics_for_subsequent_pages() async throws {
        // Given
        let totalItems = 42
        productsRemote.whenLoadingProductsForPointOfSale(siteID: siteID, thenReturn: .success(.init(items: [], hasMorePages: false, totalItems: totalItems)))

        // When
        _ = try await sut.fetchItems(pageNumber: 2)
