...
struct StorageBookingToPOSBookingMapperTests {
    ...
    /// Uses Mirror reflection to verify that every POSOrder property is explicitly
    /// populated by the booking mapper when full input data is provided.
    /// If a new property is added to POSOrder but the booking mapper doesn't set it,
    /// this test will fail because the property name won't be in the known set.
    @Test
    func test_buildPOSOrder_covers_all_POSOrder_properties() {
        // Given - provide full data so every field can be populated
        let lineItems = [
            BookingOrderLineItem(
                itemID: 10, name: "Haircut", productID: 100, variationID: 0,
                quantity: 1, price: NSDecimalNumber(string: "25.00"),
                subtotal: "25.00", total: "25.00", totalTax: "2.50",
                imageSrc: "https://example.com/img.jpg"
            )
        ]
        let refunds = [
            BookingOrderRefund(refundID: 1, reason: "Cancelled", total: "-10.00")
        ]
        let orderInfo = makeOrderInfo(
            discountTotal: "5.00",
        ...
        // Then - Use Mirror to get all property names of POSOrder
        // If a new property is added to POSOrder, it will appear here and the test will fail
        // unless it's added to the known set below.
        let knownProperties: Set<String> = [
            ...
            "formattedNetAmount",
            "datePaid",
            "lineItemQuantitiesByProductOrVariationID"
        ]

        let mirror = Mirror(reflecting: posOrder!)
        let actualProperties = Set(mirror.children.compactMap(\.label))

        let unmappedProperties = actualProperties.subtracting(knownProperties)
        #expect(
            unmappedProperties.isEmpty,
            "POSOrder has new properties not covered by StorageBookingToPOSBookingMapper: \(unmappedProperties). "
            + "Update the mapper to populate these properties from BookingOrderInfo, then add them to knownProperties."
        )
    }
