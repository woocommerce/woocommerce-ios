...
class RefundStoreTests: XCTestCase {
    ...
    // MARK: - RefundAction.previewRefund

    /// Verifies that `RefundAction.previewRefund` returns the server-calculated totals.
    ///
    func test_previewRefund_returns_server_calculated_totals() {
        // Given
        let expectation = self.expectation(description: "Preview refund")
        ...
        let action = RefundAction.previewRefund(siteID: sampleSiteID,
            ...
            case .success(let preview):
                XCTAssertEqual(preview.total, Decimal(string: "29.70"))
                XCTAssertEqual(preview.maxRefundable, Decimal(string: "50.00"))
            case .failure(let error):
                XCTFail("Expected a preview, got error: \(error)")
            }
            expectation.fulfill()
        }
    ...
    /// Verifies that `RefundAction.previewRefund` does not persist anything (preview is read-only).
    ///
    func test_previewRefund_does_not_persist_anything() {
        // Given
        let expectation = self.expectation(description: "Preview refund without persistence")
        let refundStore = RefundStore(dispatcher: dispatcher, storageManager: storageManager, network: network)
        network.simulateResponse(requestUrlSuffix: "refunds/preview", filename: "refund-preview")
        ...
        let action = RefundAction.previewRefund(siteID: sampleSiteID,
                                                orderID: sampleOrderID,
                                                lineItems: [.quantityBased(lineItemID: 50, quantity: 2)]) { _ in
            // Then
    ...
    /// Verifies that `RefundAction.previewRefund` forwards the `noRestRoute` failure that signals
    /// v4 is unavailable, so callers can fall back to v3.
    ///
    func test_previewRefund_when_route_not_registered_then_forwards_noRestRoute_failure() {
        // Given
        let expectation = self.expectation(description: "Preview refund route not registered")
        let refundStore = RefundStore(dispatcher: dispatcher, storageManager: storageManager, network: network)
        ...
        let action = RefundAction.previewRefund(siteID: sampleSiteID,
            ...
            case .failure(let error):
                XCTAssertEqual(error as? DotcomError, .noRestRoute())
            }
            expectation.fulfill()
        }
        refundStore.onAction(action)
    ...
    ///
    func test_createRefundV4_persists_created_refund() {
        // Given
        let expectation = self.expectation(description: "Create v4 refund")
        let refundStore = RefundStore(dispatcher: dispatcher, storageManager: storageManager, network: network)
        ...
        // When
        let action = RefundAction.createRefundV4(siteID: sampleSiteID,
                                                 orderID: sampleOrderID,
                                                 reason: "Damaged item",
    ...
    /// Verifies that `RefundAction.createRefundV4` forwards failures without persisting anything.
    ///
    func test_createRefundV4_when_request_fails_then_returns_failure_and_persists_nothing() {
        // Given
        let expectation = self.expectation(description: "Create v4 refund failure")
        let refundStore = RefundStore(dispatcher: dispatcher, storageManager: storageManager, network: network)
