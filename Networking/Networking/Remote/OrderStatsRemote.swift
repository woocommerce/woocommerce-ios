...
public class OrderStatsRemote: Remote {
    ...
    ///   - latestDateToInclude: The latest date to include in the results. This string should match the `granularity`, e.g.: 'day':'1955-11-05', 'week':'1955-W44', 'month':'1955-11', 'year':'1955'
    ///   - quantity: How many `unit`s to fetch
    ///   - completion: Closure to be executed upon completion.
    ///
    public func loadOrderStats(for siteID: Int, granularity: OrderStatGranularity, latestDateToInclude: String, quantity: String, completion: @escaping ([OrderStatItem]?, Error?) -> Void) {
        let path = "\(Constants.sitesPath)/\(siteID)/\(Constants.orderStatsPath)/"
        let parameters = [ParameterKeys.unit: granularity.rawValue,
