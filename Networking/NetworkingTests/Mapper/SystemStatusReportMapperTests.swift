...
/// SystemStatusReportMapper Unit Tests
///
final class SystemStatusReportMapperTests: XCTestCase {

    /// Dummy Site ID.
    ///
    private let dummySiteID: Int64 = 999999

    func test_system_status_report_fields_are_properly_parsed() throws {
        // When
    ...
    func test_system_status_report_fields_are_properly_parsed_when_response_has_no_data_envelope() throws {
        // When
    ...
    func test_system_status_report_fields_are_properly_parsed_when_response_has_inconsistent_data_type_for_page_id() throws {
        // When
...
private extension SystemStatusReportMapperTests {
    ...
    /// Returns the SystemStatusReportMapper output upon receiving `filename` (Data Encoded)
    ///
    func mapReport(from filename: String) throws -> SystemStatusReport {
        guard let response = Loader.contentsOf(filename) else {
            throw NetworkError.notFound()
        }

        return try SystemStatusReportMapper(siteID: dummySiteID).map(response: response)
    }

    /// Returns the SystemStatusReport output upon receiving `systemStatus.json`
    ///
    func mapLoadSystemStatusResponse() throws -> SystemStatusReport {
        return try mapReport(from: "systemStatus")
    }

    /// Returns the SystemStatusReport output upon receiving `systemStatus-without-data.json`
    ///
    func mapLoadSystemStatusResponseWithoutDataEnvelope() throws -> SystemStatusReport {
        return try mapReport(from: "systemStatus-without-data")
    }

    /// Returns the SystemStatusReport output upon receiving `systemStatus-inconsistent-page-id-data-type.json`
    ///
    func mapLoadSystemStatusResponseWithInconsistentPageIdDataType() throws -> SystemStatusReport {
