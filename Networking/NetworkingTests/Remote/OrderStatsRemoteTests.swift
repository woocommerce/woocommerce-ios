...
class OrderStatsRemoteTests: XCTestCase {
    ...
    func testLoadOrderStatsProperlyReturnsParsedStats() {
        ...
        remote.loadOrderStats(for: sampleSiteID, queryID: "day", unit: .day, latestDateToInclude: "1955-11-05", quantity: 31) { (orderStats, error) in
            XCTAssertNil(error)
            XCTAssertNotNil(orderStats)
            XCTAssertEqual(orderStats?.queryID, "day")
