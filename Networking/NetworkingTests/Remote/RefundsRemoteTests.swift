...
final class RefundsRemoteTests: XCTestCase {
    ...
    ///Verifies that createRefund properly parses the `order-refund` sample response.
    ///
    func testPartialRefundForSingleProductIncludingTax() {
        ...
        network.simulateResponse(requestUrlSuffix: "orders/\(sampleOrderID)/refunds", filename: "order-refund")
