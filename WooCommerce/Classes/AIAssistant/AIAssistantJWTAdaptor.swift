...
struct AIAssistantJWTAdaptor: @unchecked Sendable, AssistantJWTProviding {
    ...
    init(blogID: Int64, network: Network, credentials: Credentials?) {
        let mint: WpComJetpackAIJWTProvider.Mint = { siteID in
            try await Self.mint(siteID: siteID, network: network, credentials: credentials)
        }
    ...
    private static func mint(siteID: Int64, network: Network, credentials: Credentials?) async throws -> String {
        switch credentials {
        case .wpcom, .applicationPassword, .none:
        ...
        case .wporg:
            throw AssistantError(kind: .auth,
                                 message: Localization.unsupportedAuth)
        }
    }
    ...
    private enum Localization {
        static let unsupportedAuth = NSLocalizedString(
            "aiAssistant.jwt.error.unsupportedAuth",
            value: "Couldn't authorize the assistant on this site.",
            comment: "Error shown when the AI Assistant cannot mint a Jetpack AI token because the site uses self-hosted credentials."
        )
