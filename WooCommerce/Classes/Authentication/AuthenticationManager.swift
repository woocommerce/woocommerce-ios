...
extension AuthenticationManager: WordPressAuthenticatorDelegate {
    ...
    func shouldPresentUsernamePasswordController(for siteInfo: WordPressComSiteInfo?, onCompletion: @escaping (Error?, Bool) -> Void) {
        ...
        guard let site = siteInfo, site.hasJetpack == true else {
            let errorInfo = NSLocalizedString("Looks like you site isn't set up to use this app. Make sure your site has Jetpack installed to continue.", comment: "Error message that displays on the 'Log in by entering your site address.' screen. Jetpack is required for logging into the WooCommerce mobile apps.")
