...
private struct HardwareDetail: View {
    @Environment(\.dismiss) private var dismiss
    @State private var path: [PointOfSaleSettingsView.HardwareDest] = []

    var body: some View {
        NavigationStack(path: $path) {
            List(PointOfSaleSettingsView.HardwareDest.allCases) { dest in
            ...
            .navigationDestination(for: PointOfSaleSettingsView.HardwareDest.self) { dest in
                VStack(spacing: 16) {
