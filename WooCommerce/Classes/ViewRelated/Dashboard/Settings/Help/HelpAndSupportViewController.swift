...
final class HelpAndSupportViewController: UIViewController {
    ...
    private lazy var viewModel = HelpAndSupportViewModel(
        ...
        isAIChatEnabled: ServiceLocator.featureFlagService.isFeatureFlagEnabled(.aiSupportChat)
            && ServiceLocator.stores.sessionManager.defaultSite?.isJetpackConnected == true
    )
...
private extension HelpAndSupportViewController {
    ...
    func applicationLogWasPressed() {
        ...
        guard let applicationLogVC = UIStoryboard.settings.instantiateViewController(identifier: identifier) as? ApplicationLogViewController else {
