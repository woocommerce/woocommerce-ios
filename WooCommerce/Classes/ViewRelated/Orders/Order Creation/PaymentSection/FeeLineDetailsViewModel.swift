...
final class FeeLineDetailsViewModel: ObservableObject {
    ...
    private let feeLineTypeViewModel: FeeLineTypeViewModel
    ...
    init(isExistingLine: Bool,
        ...
        self.feeLineTypeViewModel = FeeLineTypeViewModel(isExistingLine: isExistingLine)
    }

    func removeValue() {
        if let event = feeLineTypeViewModel.removeEvent() {
...
struct FeeLineTypeViewModel: FeeOrDiscountLineTypeProtocol {
    let isExistingLine: Bool
