...
struct SelectedStoredTaxRateFetcher {
    ...
    func fetchSelectedStoredTaxRate(siteID: Int64) async -> TaxRate? {
        ...
        } catch DotcomError.resourceDoesNotExist, DotcomError.invalidResourceID {
            DDLogError("⛔️ Tax Rate with ID: \(storedTaxRateID) not found. Removing from Store Settings.")
            Task { @MainActor in
                stores.dispatch(AppSettingsAction.storeSelectedTaxRateID(id: nil, siteID: siteID))
            }
        } catch {
