...
final class WooShippingOriginAddressListViewModel: ObservableObject {
    private(set) var addresses: [WooShippingLabelAddress]
    ...
    var onSelect: ((WooShippingLabelAddress) -> Void)?

    init(addresses: [WooShippingLabelAddress],
    ...
    func isSelected(_ address: WooShippingLabelAddress) -> Bool {
        selectedAddressID == address.id
    }

    /// Selects the provided address to use as the origin address for the shipping label.
    func select(_ address: WooShippingLabelAddress) {
    ...
    func editAddress(_ address: WooShippingLabelAddress) {
        addressToEdit = WooShippingEditAddressViewModel(type: .origin, address: address, onAddressEdited: { [weak self] editedAddress in
...
extension WooShippingOriginAddressListView {
    static func sampleAddresses() -> [WooShippingLabelAddress] {
        [WooShippingLabelAddress(id: "1",
