...
final class ProductImagesUserDefaultsStatuses {
    ...
    static func addStatus(_ status: ProductImageStatus, for siteID: Int64, productID: ProductOrVariationID) {
        var statuses = getAllStatuses()
        statuses.append(status)
        saveAllStatuses(statuses)
    }

    static func removeStatus(_ status: ProductImageStatus, for siteID: Int64, productID: ProductOrVariationID) {
        var statuses = getAllStatuses()
        statuses.removeAll(where: { $0 == status })
        saveAllStatuses(statuses)
    }
