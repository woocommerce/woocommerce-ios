...
struct AdaptiveAsyncImage<Content>: View where Content: View {
    ...
    /// - Parameters:
    ///   - anyAppearanceUrl: URL for a remote image to be used for light mode.
    ...
    init(anyAppearanceUrl: URL, darkUrl: URL?, scale: CGFloat = 1, @ViewBuilder content: @escaping (AsyncImagePhase) -> Content) {
        self.anyAppearanceUrl = anyAppearanceUrl
