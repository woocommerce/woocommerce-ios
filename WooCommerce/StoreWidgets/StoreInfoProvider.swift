...
final class StoreInfoProvider: TimelineProvider {
    ...
    func loadTimeline(
        ...
        let service = StoreInfoDataService(credentials: dependencies.credentials,
                                           visitorStatsEndpoint: dependencies.visitorStatsEndpoint)
        do {
...
private extension StoreInfoProvider {
    ...
    struct Dependencies {
        ...
        let visitorStatsEndpoint: VisitorStatsEndpoint
    }
    ...
    static func fetchDependencies() -> Dependencies? {
        ...
        let storedVisitorStatsEndpoint = UserDefaults.group?[.defaultStoreVisitorStatsEndpoint] as? String
        let visitorStatsEndpoint = storedVisitorStatsEndpoint.flatMap(VisitorStatsEndpoint.init(rawValue:))
            ?? VisitorStatsEndpoint.resolve(credentials: credentials)
        return Dependencies(credentials: credentials,
                            storeID: storeID,
                            storeName: storeName,
                            storeCurrencySettings: storeCurrencySettings,
                            visitorStatsEndpoint: visitorStatsEndpoint)
    }
}
