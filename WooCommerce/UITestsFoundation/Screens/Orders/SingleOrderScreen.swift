...
public final class SingleOrderScreen: ScreenObject {
    ...
    public func verifySingleOrder(order: OrderData) throws -> Self {
        ...
        app.assertTextVisibilityCount(textToFind: order.billing.first_name, expectedCount: 2) // Check that the Mock first name ("Mira") appears twice - because this order has both billing and shipping details
        // try app.verifyChildElementOnParentCell(parent: "single-product-cell", child: "Mira") //this shouldn't work! "single-product-cell" doesn't include the text "Mira"
