...
struct WooAnalyticsEvent_AgeVerificationTests {
    ...
    @Test func test_restrictionChecked_when_given_each_result_then_maps_coarse_outcome() {
        ...
        for (result, expected) in cases {
            // When
            let event = WooAnalyticsEvent.AgeVerification.restrictionChecked(trigger: .sessionStart, decision: .allow, result: result, consentState: nil)
    ...
    @Test func test_restrictionChecked_when_given_each_consent_state_then_maps_significant_change_status() {
        ...
        for (state, expected) in cases {
            // When
            let event = WooAnalyticsEvent.AgeVerification.restrictionChecked(trigger: .sessionStart, decision: .allow, result: .unknown, consentState: state)
