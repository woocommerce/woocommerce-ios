...
final class MockCardReaderService: CardReaderService {
    ...
    let connectedReadersSubject = CurrentValueSubject<[CardReader], Never>([])
    let discoveryStatusSubject = CurrentValueSubject<CardReaderServiceDiscoveryStatus, Never>(.idle)
