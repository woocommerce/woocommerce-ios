...
struct TotalsViewHelperTests {
    ...
    // MARK: - Coupon Validation Tests

    @Test
    func test_shouldShowCouponValidation_returns_false_when_cart_has_no_coupons() {
        let cart = Cart()
        let helper = TotalsViewHelper()

        #expect(helper.shouldShowCouponValidation(
            orderState: .idle,
            paymentState: .card(.idle),
            readerConnectionStatus: .connected(.init(name: "", batteryLevel: nil)),
            cart: cart
        ) == false)
    }

    @Test
    func test_shouldShowCouponValidation_returns_true_when_order_syncing() {
        var cart = Cart()
        cart.add(.coupon(.init(id: .init(), code: "TEST10", summary: "")))
        let helper = TotalsViewHelper()
    ...
    @Test
    func test_shouldShowCouponValidation_returns_true_when_card_idle_and_reader_connected() {
        var cart = Cart()
        cart.add(.coupon(.init(id: .init(), code: "TEST10", summary: "")))
        let helper = TotalsViewHelper()
    ...
    @Test
    func test_shouldShowCouponValidation_returns_false_when_card_idle_and_reader_disconnected() {
        var cart = Cart()
        cart.add(.coupon(.init(id: .init(), code: "TEST10", summary: "")))
        let helper = TotalsViewHelper()
    ...
    @Test
    func test_shouldShowCouponValidation_returns_false_when_not_card_idle() {
        var cart = Cart()
        cart.add(.coupon(.init(id: .init(), code: "TEST10", summary: "")))
        let helper = TotalsViewHelper()
        ...
        #expect(helper.shouldShowCouponValidation(
            orderState: .idle,
            paymentState: .card(.processingPayment),
            readerConnectionStatus: .connected(.init(name: "", batteryLevel: nil)),
            cart: cart
        ) == false)
    }
    ...
    @Test
    func test_shouldShowCouponValidation_returns_false_when_cash_payment() {
        var cart = Cart()
        cart.add(.coupon(.init(id: .init(), code: "TEST10", summary: "")))
        let helper = TotalsViewHelper()
