...
final class DomainSettingsViewModelTests: XCTestCase {
    ...
    func test_onAppear_sets_freeStagingDomain_from_first_domain_matching_type() async throws {
        ...
        mockLoadDomains(result: .success([
            .init(name: "woocommerce.dm", isPrimary: false, isWPCOMStagingDomain: false, type: .mapping),
            .init(name: "woocommerce.wordpress.com", isPrimary: true, isWPCOMStagingDomain: false, type: .wpcom),
            .init(name: "woocommerce.wpcomstaging.com", isPrimary: true, isWPCOMStagingDomain: true, type: .mapping)
        ]))
        mockLoadSiteCurrentPlan(result: .success(.init(hasDomainCredit: false)))
        ...
        // Then
        XCTAssertEqual(viewModel.freeStagingDomain, .init(isPrimary: true, name: "woocommerce.wordpress.com"))
    }

    func test_onAppear_sets_freeStagingDomain_to_nil_when_no_domain_matches_isWPCOMStagingDomain_and_type() async throws {
        // Given
        mockLoadDomains(result: .success([
            .init(name: "woocommerce.com", isPrimary: true, isWPCOMStagingDomain: false, type: .mapping)
        ]))
        mockLoadSiteCurrentPlan(result: .success(.init(hasDomainCredit: false)))
    ...
    func test_onAppear_sets_domains_to_non_wpcom_domains() async throws {
        // Given
        mockLoadDomains(result: .success([
            .init(name: "woocommerce.wpcomstaging.com", isPrimary: true, isWPCOMStagingDomain: true, type: .wpcom),
            .init(name: "woocommerce.com", isPrimary: true, isWPCOMStagingDomain: true, type: .mapping),
            .init(name: "woocommerce.wordpress.com", isPrimary: true, isWPCOMStagingDomain: false, type: .wpcom),
        ]))
        mockLoadSiteCurrentPlan(result: .success(.init(hasDomainCredit: false)))
        ...
        // Then
        XCTAssertEqual(viewModel.domains, [.init(isPrimary: true, name: "woocommerce.com", autoRenewalDate: nil)])
    }
}
