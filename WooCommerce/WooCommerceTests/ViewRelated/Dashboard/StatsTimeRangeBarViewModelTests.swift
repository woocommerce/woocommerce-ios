...
class StatsTimeRangeBarViewModelTests: XCTestCase {
    ...
    func testTodayText() {
        ...
        let startDate = Date(timeIntervalSince1970: 1_565_892_875)
        // GMT: Friday, August 16, 2019 2:14:35 AM
        let endDate = Date(timeIntervalSince1970: 1_565_921_675)
        let timezone = TimeZone(identifier: "GMT") ?? .current
        let viewModel = StatsTimeRangeBarViewModel(
            startDate: startDate,
            endDate: endDate,
            timeRange: .today,
            timezone: timezone)
    ...
    func testThisWeekText() {
        ...
        let startDate = Date(timeIntervalSince1970: 1_564_272_000)
        // GMT: Saturday, August 3, 2019 11:59:59 PM
        let endDate = Date(timeIntervalSince1970: 1_564_876_799)
        let timezone = TimeZone(identifier: "GMT") ?? .current
        let viewModel = StatsTimeRangeBarViewModel(
            startDate: startDate,
            endDate: endDate,
        ...
        formatter.timeZone = timezone
        let expectedText = String.localizedStringWithFormat(
