...
final class ReviewOrderViewModelTests: XCTestCase {
    ...
    func test_productDetailsCellViewModel_returns_correct_item_details() {
        ...
        let product = ProductListItem.fake().copy(productID: productID)
    ...
    func test_productDetailsCellViewModel_returns_correct_hasAddOns_if_view_model_receives_showAddOns_as_true_and_there_are_valid_addons_from_attributes() {
        ...
        let product = ProductListItem.fake().copy(productID: productID, addOns: [addOn])
    ...
    func test_product_section_does_not_contain_refunded_items() {
        ...
        let product1 = ProductListItem.fake().copy(productID: productID)
        let product2 = ProductListItem.fake().copy(productID: productID2)
    ...
    func test_customerSection_does_not_contain_shipping_address_cell_if_there_are_only_virtual_products() {
        ...
        let item = OrderItem.fake().copy(productID: productID)
        let order = Order.fake().copy(status: .processing, items: [item])
        let product = ProductListItem.fake().copy(productID: productID, virtual: true)
    ...
    func test_customerSection_does_not_contain_shipping_address_cell_if_there_is_virtual_product_and_shipping_address() {
        ...
        let product = ProductListItem.fake().copy(productID: productID, virtual: false)
