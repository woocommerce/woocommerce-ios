...
class LoginLinkRequestViewController: LoginViewController {
    ...
    @objc func requestAuthenticationLink() {
        ...
        guard email.isValidEmail() else {
            ...
            let alert = UIAlertController(title: NSLocalizedString("Can Not Request Link", comment: "This text appears as the title of an error alert dialog that is shown when a user tries to request a magic authentication link but has entered an invalid email address during the login process."), message: NSLocalizedString("A valid email address is needed to mail an authentication link. Please return to the previous screen and provide a valid email address.", comment: "This text appears as an alert message in the WordPress authentication flow when a user attempts to request a magic login link but has entered an invalid email address. The alert includes 'Need help?' and 'OK' action buttons and instructs the user to go back and enter a valid email."), preferredStyle: .alert)
