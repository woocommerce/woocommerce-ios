...
@objc public enum WordPressOrgXMLRPCValidatorError: Int, Error {
    ...
    public var localizedDescription: String {
        ...
        case .invalid:
            return NSLocalizedString("Couldn't connect to the WordPress site. There is no valid WordPress site at this address. Check the site address (URL) you entered.", comment: "Error message displayed when a user tries to connect to a website that exists but is not a WordPress site during the site setup process. This appears as an alert or error dialog to inform the user that the URL they entered doesn't point to a valid WordPress installation.")
        case .blocked:
            return NSLocalizedString("Couldn't connect. Your host is blocking POST requests, and the app needs that in order to communicate with your site. Please contact your hosting provider to solve this problem.", comment: "This is an error message displayed to users when they attempt to connect their self-hosted WordPress site to the WooCommerce app, but their web hosting provider is blocking POST requests that the app needs for communication. The message appears when the server returns a 405 error during the site connection process.")
        case .forbidden:
            return NSLocalizedString("Couldn't connect. We received a 403 error when trying to access your site's XMLRPC endpoint. The app needs that in order to communicate with your site. Please contact your hosting provider to solve this problem.", comment: "Error message displayed when the app fails to connect to a WordPress site due to a 403 forbidden error while accessing the XMLRPC endpoint. This technical error message appears in an alert or error dialog during the site setup process, explaining why the connection failed and directing users to contact their hosting provider.")
        case .xmlrpc_missing:
            return NSLocalizedString("Couldn't connect. Required XML-RPC methods are missing on the server. Please contact your hosting provider to solve this problem.", comment: "Error message displayed when attempting to connect to a self-hosted WordPress site that has RSD link present but is missing required XML-RPC methods on the server. This appears as an alert or error dialog when the connection validation fails during site setup.")
        }
    }
}
...
open class WordPressOrgXMLRPCValidator: NSObject {
    ...
    private func urlForXMLRPCFromURLString(_ urlString: String, addXMLRPC: Bool) throws -> URL {
        ...
        if baseURL.lastPathComponent != "xmlrpc.php" && addXMLRPC {
            ...
            // WPAuthenticatorLoggingInfo("Assume the given url is the home page and XML-RPC sits at /xmlrpc.php")
    ...
    private func guessXMLRPCURLFromHTMLURL(_ htmlURL: URL,
                                           success: @escaping (_ xmlrpcURL: URL) -> Void,
                                           failure: @escaping (_ error: NSError) -> Void) {
        // WPAuthenticatorLoggingInfo("Fetch the original url and look for the RSD link by using RegExp")
    ...
    private func guessXMLRPCURLFromRSD(_ rsd: String,
                                       success: @escaping (_ xmlrpcURL: URL) -> Void,
                                       failure: @escaping (_ error: NSError) -> Void) {
        // WPAuthenticatorLoggingInfo("Parse the RSD document at the following URL: \(rsd)")
        ...
        let dataTask = session.dataTask(with: rsdURL, completionHandler: { (data, _, error) in
            ...
            // WPAuthenticatorLoggingInfo("Bingo! We found the WordPress XML-RPC element: \(xmlrpcURL)")
