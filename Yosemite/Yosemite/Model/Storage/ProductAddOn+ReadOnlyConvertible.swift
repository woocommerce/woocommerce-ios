...
extension Storage.ProductAddOn: ReadOnlyConvertible {
    ...
    public func toReadOnly() -> Yosemite.ProductAddOn {
        ...
        return ProductAddOn(type: AddOnType(rawValue: type) ?? .checkbox,
                            display: AddOnDisplay(rawValue: display) ?? .none,
                            name: name,
                            titleFormat: AddOnTitleFormat(rawValue: titleFormat) ?? .none,
                            descriptionEnabled: descriptionEnabled,
                            description: descriptions,
                            required: Int(required),
                            position: Int(position),
                            restrictions: Int(restrictions),
                            restrictionsType: AddOnRestrictionsType(rawValue: restrictionsType) ?? .none,
                            adjustPrice: Int(adjustPrice),
                            priceType: AddOnPriceType(rawValue: priceType) ?? .none,
                            price: price,
                            min: Int(min),
                            max: Int(max),
