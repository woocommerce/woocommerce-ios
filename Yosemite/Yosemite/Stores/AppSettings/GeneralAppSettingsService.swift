...
public struct GeneralAppSettingsService {
    ...
    public init(fileStorage: FileStorage, fileURL: URL) {
    ...
    public init() {
        self.init(fileStorage: PListFileStorage(), fileURL: Self.defaultFileURL)
    }

    /// Updates the stored settings with the given settings object
    ///
    public func update(settings: GeneralAppSettings) throws {
    ...
    ///
    public func value<Value>(for keyPath: KeyPath<GeneralAppSettings, Value>) -> Value {
        settings[keyPath: keyPath]
    }

    /// Returns a publisher that emits changes when the setting for the specified key path changes
    ///
    public func publisher<Value: Equatable>(for keyPath: KeyPath<GeneralAppSettings, Value>) -> AnyPublisher<Value, Never> {
        settingsPublisher
            .map(keyPath)
            .removeDuplicates()
            .eraseToAnyPublisher()
    ...
    ///
    public func update<Value>(_ value: Value, for keyPath: WritableKeyPath<GeneralAppSettings, Value>) throws {
    ...
    ///
    public func pluck<Key, Value>(from keyPath: KeyPath<GeneralAppSettings, [Key: Value]>, key: Key) -> Value? {
    ...
    ///
    public func patch<Key, Value>(_ value: Value, into keyPath: WritableKeyPath<GeneralAppSettings, [Key: Value]>, key: Key) throws {
...
private extension GeneralAppSettingsService {
    ...
    static let defaultFilename = "general-app-settings.plist"

    static var defaultFileURL: URL {
        let documents = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first
        return documents!.appendingPathComponent(defaultFilename)
    }
}
