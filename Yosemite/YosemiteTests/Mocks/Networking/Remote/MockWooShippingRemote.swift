...
final class MockWooShippingRemote {
    ...
    private var updateShipment = [ResultKey: Result<[String: [WooShippingShipment]], Error>]()
    ...
    func whenUpdatingShipment(siteID: Int64,
                              thenReturn result: Result<[String: [WooShippingShipment]], Error>) {
        let key = ResultKey(siteID: siteID)
...
extension MockWooShippingRemote: WooShippingRemoteProtocol {
    ...
    func updateShipment(siteID: Int64,
                        ...
                        completion: @escaping (Result<[String: [WooShippingShipment]], any Error>) -> Void) {
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
