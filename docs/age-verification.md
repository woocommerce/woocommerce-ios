...
|---|---|---|
| `account_age_restriction_checked` | One per completed verification flow. Volume guardrail: only when the account is under a covered regime or the decision is not a plain allow, so it is never sent for non-covered accounts or unsupported OS versions. | `age_range_outcome` (`eligible`, `age_13_17`, `below_13`, `declined_sharing`, `unavailable`, `not_applicable`, `invalid_ui_state`, `sdk_error`, `unknown`), `final_decision` (`allowed`, `restricted`, `wall_consent_required`, `wall_consent_pending`, `wall_consent_denied`), `restriction_reason` when restricted (`below_minimum_age`, `consent_required`, `consent_pending`, `consent_denied`), `significant_change_status` when the consent branch ran (`not_applicable`, `required`, `approved`, `pending`, `declined`, `unavailable`), `sdk_error_domain` + `sdk_error_code` for SDK failures |
...
| `account_age_consent_requested` | iOS-only: the app handed the consent question to the system | `change_type` (`age_rating`, `manual`), `result` (`sent`, `not_available`, `failed`), `is_reask` |
...
The first three events and their property names match Android (`woocommerce-android` PR #16377). iOS requests age gates 13 and 18 only, so it reports `age_13_17` where Android splits `13_15` / `16_17`; Android's `access_status`, `request_stage`, `retry_count` and `is_recovery` have no iOS equivalent.
