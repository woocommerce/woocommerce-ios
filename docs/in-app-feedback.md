...
The feature is designed so that when we have new banners (messages) like the order form shipping lines banner, we just have to add a new option in [`FeedbackType`](../Storage/Storage/Model/FeedbackType.swift). This `FeedbackType` is used by `InAppFeedbackCardVisibilityUseCase` to determine whether to show the buttons or not.
